...
\begin{algorithm}[H]
	...
	\While{Flowtable \textbf{Match} $TCP\_package$ from h1}{
	\textbf{Send} $TCP\_package$ to $port~2$ and its copy to $Ryu Controller$ \;
	}

	\textbf{CONTROL PLANE}[Software Layer]\;
	\While{\textbf{Recieve} $TCP\_package$}{
		\If{$TCP\_package.source\_mac \notin White List$}{
			\If{$TCP\_package.ACK \leq 1 $}{
				\textbf{Pass} $TCP\_package$ \;

				\textbf{Send} Flow Entry with $hard\_timeout=60$ to \textbf{Drop} $HTTP~Package$ from $TCP\_package.source\_mac$ to $h2.mac$\;

				$White~List \Leftarrow White~List \cup \{TCP\_package.source\_mac\}$ \;
			}
		}
	}
